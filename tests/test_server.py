...
@pytest.mark.skipif(sys.platform == "win32",
                    reason="HTTPServer.handle_request() blocks indefinitely on Windows")
...
class TestAgentAPIHandler:
    ...
    def test_tools_endpoint(self, config):
        """GET /tools should return tool definitions list."""
        from ata_coder.server import create_server
        import urllib.request
        ...
        try:
            resp = urllib.request.urlopen(f"http://127.0.0.1:{port}/tools")
            data = json.loads(resp.read().decode())
            assert resp.status == 200
    ...
    def test_skills_endpoint(self, config):
        """GET /skills should return skills list."""
        from ata_coder.server import create_server
        import urllib.request
        ...
        try:
            resp = urllib.request.urlopen(f"http://127.0.0.1:{port}/skills")
            data = json.loads(resp.read().decode())
            assert resp.status == 200
    ...
    def test_sessions_endpoint(self, config):
        """GET /sessions should return sessions list."""
        from ata_coder.server import create_server
        import urllib.request
        ...
        try:
            resp = urllib.request.urlopen(f"http://127.0.0.1:{port}/sessions")
            data = json.loads(resp.read().decode())
            assert resp.status == 200
    ...
    def test_cors_preflight(self, config):
        ...
        try:
            req = urllib.request.Request(
                f"http://127.0.0.1:{port}/chat", method="OPTIONS"
            )
