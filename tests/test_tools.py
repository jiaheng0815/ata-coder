...
class TestToolResult:
    def test_success_to_message(self):
        r = ToolResult(success=True, output="hello")
        assert r.to_message() == "hello"

    def test_failure_to_message(self):
...
class TestToolReadFile:
    ...
    def test_read_existing_file(self):
        ...
        f.write_text("line1\nline2\nline3\n", encoding="utf-8")
        result = self.executor.execute("read_file", {"file_path": str(f)})
    ...
    def test_read_directory_fails(self):
        result = self.executor.execute("read_file", {"file_path": self.tmp})
        assert not result.success
    ...
    def test_read_caches_file(self):
        f = Path(self.tmp) / "cache_test.txt"
        f.write_text("cached content\n", encoding="utf-8")
        cache_key = str(f.resolve())
        # First read
        r1 = self.executor.execute("read_file", {"file_path": str(f)})
        assert r1.success
        # Second read — should be from cache
        r2 = self.executor.execute("read_file", {"file_path": str(f)})
        assert r2.success
...
class TestToolEditFile:
    ...
    def test_simple_replacement(self):
        ...
        result = self.executor.execute("edit_file", {
            "file_path": str(fp),
            "old_string": "hello",
            "new_string": "hi",
        })
    ...
    def test_duplicate_string_rejected(self):
...
class TestToolRunShell:
    ...
    def test_simple_command(self):
        result = self.executor.execute("run_shell",
                                        {"command": "echo hello"})
        assert result.success
...
class TestToolGrep:
    ...
    def test_find_pattern(self):
        ...
        result = self.executor.execute("grep",
...
class TestToolGlob:
    ...
    def test_find_py_files(self):
        ...
        result = self.executor.execute("glob",
...
class TestToolListDir:
    ...
    def test_list_directory(self):
        (Path(self.tmp) / "a.py").touch()
        (Path(self.tmp) / "b.py").touch()
        result = self.executor.execute("list_dir", {"path": self.tmp})
        assert result.success
        assert "a.py" in result.output
        assert "b.py" in result.output
...
class TestToolEditCallback:
    ...
    def test_edit_callback_invoked(self):
        calls = []
        ...
        self.executor.execute("edit_file", {
            "file_path": str(fp),
            "old_string": "original",
            "new_string": "modified",
        })
    ...
    def test_edit_callback_not_called_on_read(self):
        calls = []
        ...
        self.executor.execute("read_file", {"file_path": str(f)})
