...
def test_chat_stream_yields_text():
    ...
    with patch.object(client._client, 'send', return_value=FakeResponse(200, sse_lines)):
        results = list(client.chat_stream([{"role": "user", "content": "hi"}]))
...
def test_chat_stream_yields_tool_calls():
    ...
    with patch.object(client._client, 'send', return_value=FakeResponse(200, sse_lines)):
        results = list(client.chat_stream([{"role": "user", "content": "greet world"}]))
...
def test_chat_stream_empty_response_raises():
...
def test_chat_stream_retries_on_429():
    ...
    def fake_send(request, stream=False):
        call_count[0] += 1
        if call_count[0] < 3:
...
def test_chat_stream_raises_on_401():
