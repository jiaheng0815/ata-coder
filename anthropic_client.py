"""
Anthropic Messages API client — provider-agnostic.
...
Configuration:
  OPENAI_BASE_URL  → base URL (auto-appends /anthropic if needed)
...
class AnthropicClient:
    """HTTP client for Anthropic-compatible Messages API. Provider-agnostic."""
    ...
    def __init__(self, config: LLMConfig | None = None):
        ...
        if map_json:
            try:
                ...
                self._model = model_map.get(self._model, self._model)
    ...
    def chat(self, messages: list[Message], system_prompt: str = "",
        ...
        return self._request_with_retry(body)

    def _request_with_retry(self, body: dict[str, Any]) -> Message:
        """Send request with exponential backoff on rate limits / server errors."""
        last_error = None
        for attempt in range(self._max_retries + 1):
            try:
                resp = self._client.post(self._api_url, json=body)
            ...
            try:
                ...
                try:
                    ...
                    err_msg = str(e)
                raise RuntimeError(f"Anthropic API error ({resp.status_code}): {err_msg}")
    ...
    def chat_stream(self, messages: list[Message], system_prompt: str = "",
                    tools: list[ToolDef] | None = None) -> Iterator[tuple[str, Any]]:
        tool_defs = tools if tools is not None else self._tools
        anthropic_msgs, system = self._convert_messages(messages, system_prompt)
        ...
        with self._client.stream("POST", self._api_url, json=body) as resp:
            resp.raise_for_status()
    ...
    def _convert_response(self, data: dict) -> Message:
        ...
        for block in data.get("content", []):
            ...
            elif t == "thinking": reasonings.append(block.get("thinking", ""))

        result["content"] = "\n".join(texts)
        if tools: result["tool_calls"] = tools
        if reasonings: result["reasoning_content"] = "\n".join(reasonings)
        ...
        if self._usage_callback: self._usage_callback(inp, out or 1)
