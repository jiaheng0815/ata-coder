...
class FileChange:
    ...
    def diff(self) -> str:
        ...
        diff = difflib.unified_diff(
            old, new,
            fromfile=f"a/{self.file_path}" if self.old_content else f"/dev/null",
            tofile=f"b/{self.file_path}" if self.new_content else f"/dev/null",
        )
        return "".join(diff)
...
class ChangeTracker:
    ...
    def _backup(self, file_path: str) -> str:
        ...
        backup_name = f"{path.name}.{time.strftime('%Y%m%d-%H%M%S')}.bak"
    ...
    def _apply_revert(self, c: FileChange) -> None:
        ...
        if c.change_type == ChangeType.WRITE:
            if c.old_content is None:
                ...
                path.write_text(c.old_content, encoding="utf-8")
            ...
            if c.new_content is not None:
                ...
                path.write_text(c.new_content, encoding="utf-8")
