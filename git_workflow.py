...
class GitWorkflow:
    ...
    def get_status(self) -> GitStatus:
        ...
        if code == 0 and output:
            for line in output.split("\n"):
                # CRITICAL: do NOT strip the line. git status --short uses
                # positional encoding: column 0 = index (staged), column
                # 1 = working tree (unstaged).  " M path" (unstaged) vs
                # "M  path" (staged) must remain distinguishable.
                line = line.rstrip("\n\r")
                if not line.strip():
                    continue
                # Untracked: "?? path"
                if len(line) >= 2 and line[0] == "?" and line[1] == "?":
                    ...
                    # Column 0 (index / staging area)
                    if len(line) > 0 and line[0] in "MADRC":
                        status.staged += 1
    ...
    def get_changed_files(self) -> list[str]:
        ...
        Returns relative file paths, excluding deleted files.
        """
        code, output, _ = _run_git(["diff", "--name-only", "HEAD"], self.cwd)
        if code != 0 or not output:
            # No commits yet (or diff failed) — list tracked + untracked files
            # but exclude .gitignored paths. Use --diff-filter to only show
            # files that exist (exclude deleted).
            code2, output2, _ = _run_git(
                ["diff", "--name-only", "--diff-filter=ACMRTUXB", "HEAD"], self.cwd
            )
            if code2 == 0 and output2:
                output = output2
            else:
                # Truly fresh repo: use ls-files but NOT --cached (which
                # returns every tracked file in the index) — just untracked.
                _, output, _ = _run_git(
                    ["ls-files", "--others", "--exclude-standard"], self.cwd
                )
        if not output:
            return []
