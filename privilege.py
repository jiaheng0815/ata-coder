...
def detect_privilege() -> PrivilegeLevel:
    ...
    if os_family == OSFamily.WINDOWS:
        try:
            ...
            try:
                test_path = "C:\\Windows\\Temp\\ata_coder_admin_test"
...
def get_elevation_prefix() -> str | None:
    ...
    if os_family == OSFamily.LINUX:
        ...
        # Also sudo works on macOS

    elif os_family == OSFamily.WINDOWS:
        # Windows: start /b /wait with elevated prompt
        # Can use PowerShell Start-Process -Verb RunAs
        return 'powershell -Command "Start-Process -Verb RunAs -Wait -FilePath \'{cmd}\'"'
...
"""
    ...
    def check_dangerous_command(self, command: str) -> tuple[bool, str]:
        ...
        needs_elevation = self._needs_elevation(command)
        if needs_elevation:
            return False, (
    ...
    def _needs_elevation(self, command: str) -> bool:
